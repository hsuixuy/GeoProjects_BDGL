...
export class Evaluator {
	...
	evaluate(a, b, operation, targetBrush = new Brush()) {
		...
		a.prepareGeometry();
		b.prepareGeometry();
		// constructionTime += window.performance.now() - startTime;
		// console.log(`constructionTime：${constructionTime}`)

		const { triangleSplitter, attributeData, attributes, useGroups, debug } =
			this;
		...
		if (useGroups) {
			...
			for (let i = 0, l = allMaterials.length; i < l; i++) {
				const foundGroup = Boolean(
					groups.find((group) => group.materialIndex === i)
				);
				if (!foundGroup) {
					allMaterials[i] = null;
				} else {
