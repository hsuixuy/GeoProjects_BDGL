...
export class TriangleSplitter {
		...
		if (Array.isArray(tri)) {
			for (let i = 0, l = tri.length; i < l; i++) {
				...
				if (i === 0) {
					...
					throw new Error(
						"Triangle Splitter: Cannot initialize with triangles that have different normals."
					);
				}
	...
	splitByTriangle(triangle) {
		...
		if (
			Math.abs(1.0 - Math.abs(_plane.normal.dot(normal))) < COPLANAR_EPSILON
		) {
	...
	splitByPlane(plane, triangle = null, coplanarIndex = -1) {
		...
		for (let i = 0, l = triangles.length; i < l; i++) {
			...
			for (let t = 0; t < 3; t++) {
				...
				if (
					Math.abs(startDist) < COPLANAR_EPSILON &&
					Math.abs(endDist) < COPLANAR_EPSILON
				) {
					coplanarEdge = true;
					break;
				}
			...
			if (intersects === 2 && _foundEdge.distance() > COPLANAR_EPSILON) {
				if (vertexSplitEnd !== -1) {
					...
					if (
						arr[nextVert1].distanceToSquared(_foundEdge.start) <
						arr[nextVert2].distanceToSquared(_foundEdge.end)
					) {
